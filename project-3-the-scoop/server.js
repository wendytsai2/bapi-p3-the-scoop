...
function getOrCreateComment(url, request){
  ...
  if (comment){
    console.log('>>>> comment var already exists');
    response.body = {id: database.comments[id]};
    response.status = 200;
  } else if (requestComment && requestComment.username && database.users[requestComment.username] &&
